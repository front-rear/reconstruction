...
class Trainer(TrainerBase):
    ...
    def eval(self):
        ...
        for scale in self.val_scales_list:
            ...
            self.logger.info(f"scale_{scale} has {max(labels)+1} groups, mesh_voting: {self.mesh_voting}")
            ...
            if self.mesh_voting:
                ...
                self.logger.info(f"shape of mesh_group {mesh_group.shape}")
                ...
                self.logger.info(f"Mesh has {len(mesh.faces)} faces")
