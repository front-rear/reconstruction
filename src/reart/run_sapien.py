...
def main(args):
    ...
    if args.use_flow_loss:
        ...
        complete_pc_list = torch.from_numpy(sample['complete_pc_list']).float().to(device)  # [T, N, 3]
        flow_ref_list, _ = compute_flow_list(flow_model, complete_pc_list)  # [T-1, N, 3]
        pc_ref_list = complete_pc_list[:-1]  # [T-1, N, 3]
    ...
    if args.model == "base":
        ...
        if args.resume is None:
            ...
            print(f"joint connection: {joint_connection.shape}\n {joint_connection}")
            ...
            root_part = 0  # use the first part (unmoved part) as the root
            root_trans = trans_list[:, root_part]
            ...
            G, root_part, axis_list, moment_list, theta_list, distance_list, edge_index, joint_type_list = build_graph(
                ...
                cano_points=cano_pc, seg_part=seg_part, enable_reject=args.check_revolute_joint_position)
    ...
    for i in tqdm(range(n_iter)):
        ...
        if not args.ablation:
            kwargs["gt_seg"] = complete_gt_part_list[0].clone()
        ...
        if not args.evaluate:
            ...
            if args.use_flow_loss:
                with torch.no_grad():
                    query_list = torch.cat((pc_trans_list[:dataset.cano_idx], cano_pc[None], pc_trans_list[dataset.cano_idx:]), dim=0)[:-1]  # Put back the cano frame, but exclude the last frame
                    pairwise_flow_list = feature_propagation(query_list, pc_ref_list, flow_ref_list, False)  # [T-1, N, 3]

                complete_pred_pc_list = torch.cat((pc_trans_list[:dataset.cano_idx], cano_pc[None], pc_trans_list[dataset.cano_idx:]), dim=0)  # [T, N, 3]
                pred_flow_list = complete_pred_pc_list[1:, :, :] - complete_pred_pc_list[:-1, :, :]  # [T-1, N, 3]
                ...
                if i == n_iter - 1:
                    pcds = []
                    for j in range(0, 2):
                        pcd = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(complete_pred_pc_list[j].detach().cpu().numpy()))
                        pcd.paint_uniform_color(COLORS[j])
                        pcds.append(pcd)
                    print("Final predicted point clouds:")
                    o3d.visualization.draw_geometries(pcds)

                if i == n_iter - 1 and False:
                    # visualize predicted flow
                    # flow_ref_list_np = torch.cat([complete_pc_list[0].cpu().unsqueeze(0), pred_flow_list[0].detach().cpu().unsqueeze(0)], dim=0).numpy()
                    x = np.array([complete_pc_list[1].cpu().numpy() - pred_flow_list[0].detach().cpu().numpy(), complete_pc_list[1].cpu().numpy()])
                    n_intervals, n_points, _ = x.shape
                    ...
                    line_set.points = o3d.utility.Vector3dVector(x.reshape(-1, 3))
                    line_set.lines = o3d.utility.Vector2iVector(np.array(lines))
                    line_set.colors = o3d.utility.Vector3dVector(colors)

                    # 创建点云对象
                    point_cloud = o3d.geometry.PointCloud()
                    point_cloud.points = o3d.utility.Vector3dVector(x.reshape(-1, 3))
                    point_cloud.colors = o3d.utility.Vector3dVector(
                        np.array(
                            [[1, 0, 0]] * n_points + [[1, 0.8, 0]] * n_points + [[0, 0.8, 0]] * n_points + [[0, 0.8, 1]] * n_points,
                            dtype=np.float64
                        )
                    )
        ...
        if i % args.snapshot_gap == 0 or i == n_iter - 1:
            ...
            if i == n_iter - 1:
                if not args.evaluate:  # use the latest model weight after optimizer step
                    with torch.no_grad():
                        if args.ablation:
                            _, seg_part, trans_list = model(cano_pc, show=True, complete_pc_list=complete_pc_list)
                        else:
                            _, seg_part, trans_list = model(cano_pc, gt_seg=complete_gt_part_list[0].clone(), show=False, complete_pc_list=complete_pc_list)
                        
                    # Save cano_pc colored with seg_part
                    pcd = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(cano_pc.detach().cpu().numpy()))
                    pcd.colors = o3d.utility.Vector3dVector(COLORS[seg_part.detach().cpu().numpy()])
                    pcd.estimate_normals()
                    # o3d.visualization.draw_geometries([pcd])
                    o3d.io.write_point_cloud(os.path.join(save_dir, "cano_pc.ply"), pcd)
...
if __name__ == "__main__":
    ...
    # network
    parser.add_argument("--num_parts", default=3, type=int, help="seg MLP number of parts")
    ...
    parser.add_argument("--lambda_joint", default=10, type=float, help="joint cost/loss weight")
    parser.add_argument("--check_revolute_joint_position", action="store_true", help="reject revolute joints outside the child object")
