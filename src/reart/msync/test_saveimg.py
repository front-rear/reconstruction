...
def remove_motion_outliers(motion_absolute, xyz):
    ...
    for view_i in range(n_view):
        ...
        gsegm = scipy.stats.mode(nb_segm, axis=1).mode  # (N, )
...
if __name__ == '__main__':
    ...
    parser.add_argument('config', type=str, default="config/articulated-full.yaml", help='Config files')
    ...
    with tqdm.tqdm(enumerate(test_loader, 0), total=len(test_loader), desc='test') as tbar:
        for i, data in tbar:
            with torch.no_grad():
                ...
                vis.create_window()
                for pcd in segmented_pcds: vis.add_geometry(pcd)
                # vis.update_geometry()
                # vis.update_renderer()
                # vis.poll_events()
                vis.capture_screen_image("images/{:05d}.png".format(i), True)
                vis.destroy_window()
