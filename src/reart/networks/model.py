...
class BaseModel(nn.Module):
    def __init__(self, num_parts, pose_len, joint_trajectory=None, init_6d=None, init_t=None):
        ...
        if joint_trajectory is None:
            if init_6d is None:
                ...
                self.proposal_6d = nn.Parameter(init_6d, requires_grad=False)  # [T-1, P, 6]
    ...
    def forward(self, cano_pc, **kwargs):
        ...
        input = cano_pc.permute(1, 0).unsqueeze(dim=0) # [1, 3, N]
        if "gt_seg" in kwargs:
            gt_seg = kwargs["gt_seg"]
            weight = F.one_hot(gt_seg, num_classes=self.num_parts).float()  # [N, P]
        else:
            seg = self.seg_head(input).squeeze(dim=0).permute(1, 0)  # [N, P]
        ...
        pc_trans_list = torch.bmm(cano_pc, rotation.transpose(1, 2)) + translation[:, None, :]  # [T-1 * P, N, 3] Transformed canonical pc
        ...
        # Visualize the transformed canonical pc of frame 0
        if "show" in kwargs and kwargs["show"]:
            p0 = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(kwargs["complete_pc_list"][0].detach().cpu().numpy()))
            p0.paint_uniform_color([1, 0, 0])
            pcds = [p0]
            for p in range(self.num_parts):
                if "gt_seg" in kwargs:
                    pcd = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(pc_trans_list[0, p, gt_seg == p].detach().cpu().numpy()))
                else:
                    pcd = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(pc_trans_list[0, p, seg.argmax(dim=-1) == p].detach().cpu().numpy()))
                pcd.paint_uniform_color(COLORS[p])
                pcds.append(pcd)
            print("Visualizing the transformed canonical pc of frame 0 (Pass 1)")
            o3d.visualization.draw_geometries(pcds)

        pc_trans_list = (weight.permute(1, 0)[None, :, :, None] * pc_trans_list).sum(dim=1)  # [T-1, N, 3]
        if "gt_seg" in kwargs:
            return pc_trans_list, gt_seg, trans_list
        else:
...
class KinematicModel(nn.Module):
    ...
    def forward(self, input_pc, **kwargs):
        ...
        if "show" in kwargs and kwargs["show"]:
            ...
            for p in range(self.num_parts):
                if "gt_seg" in kwargs:
                    pcd = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(pc_trans_list[0, p, kwargs["gt_seg"] == p].detach().cpu().numpy()))
                else:
                    pcd = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(pc_trans_list[0, p, seg_part == p].detach().cpu().numpy()))
                pcd.paint_uniform_color(COLORS[p])
                pcds.append(pcd)
            print("Visualizing the transformed canonical pc of frame 0 (Pass 2)")
            o3d.visualization.draw_geometries(pcds)
