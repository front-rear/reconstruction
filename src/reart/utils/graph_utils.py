...
def rot_translation_loss(trans_a: torch.Tensor, trans_b: torch.Tensor):
    # trans_a, trans_b: (T, 4, 4)
    # ret: (T, )

    mult_rotation = torch.bmm(trans_a[:, :3, :3], trans_b[:, :3, :3].transpose(2, 1))
    rot = torch.acos(torch.clamp((torch.diagonal(mult_rotation, dim1=-2, dim2=-1).sum(dim=-1) - 1) / 2, -1 + 1e-6, 1 - 1e-6))
    translation = torch.norm((trans_b[:, :3, 3] - trans_a[:, :3, 3]), dim=-1)
...
def compute_geo_cost(rel_trans, axis, moment, theta, distance):
    ...
    geo_cost_1 = rot_translation_loss(T_recon.reshape(-1, 4, 4), rel_trans_.reshape(-1, 4, 4))
    ...
    geo_cost = torch.min(geo_cost_r, geo_cost_1)
    return geo_cost
...
def mst_wrapper(seg_part, trans, cano_pc, chamfer_dist, verbose=False, num_fps=20,
    ...
    print("geo_cost: ", geo_cost)
    print("dist_cost: ", dist_cost)
    print("joint_cost: ", joint_cost)
