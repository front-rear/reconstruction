...
from utils.graph_utils import compute_root_cost, compute_mean_screw_param, frobenius_cost, rot_translation_loss
...
def build_graph(edges_list, trans_list, verbose=False, root_part=None,
                revolute_only=True, return_joint_type=False, cano_points=None, seg_part=None, enable_reject=False):
    ...
    for idx, edge in enumerate(G.edges()):
        ...
        if revolute_only: # robot
            ...
            geo_cost_r = rot_translation_loss(T_recon.reshape(-1, 4, 4), rel_trans.reshape(-1, 4, 4))
            ...
            geo_cost_p = rot_translation_loss(T_recon.reshape(-1, 4, 4), rel_trans.reshape(-1, 4, 4))
            geo_cost_p = geo_cost_p.sum()

            print("geo_cost_p: {}, geo_cost_r: {}".format(geo_cost_p, geo_cost_r))

            # Check whether the revolute axis is within the bounding box of the child part
            # if not, use prismatic joint instead
            rev_axis = mean_axis.reshape(1, 3)
            rev_moment = mean_moment.reshape(1, 3)
            rev_origin = torch.cross(rev_axis, rev_moment)
            print("rev_axis: {}, rev_moment: {}, rev_origin: {}".format(rev_axis, rev_moment, rev_origin))
            r1 = torch.cat([rev_origin, rev_axis], dim=1).cpu().numpy()
            r2 = torch.cat([rev_origin, -rev_axis], dim=1).cpu().numpy()
            rays = o3d.core.Tensor(np.array([r1, r2]), dtype=o3d.core.Dtype.Float32)
            assert cano_points is not None and seg_part is not None

            # Create AABB for pc_list
            EXTEND = 0.1
            min_val = cano_points[seg_part == child_part_id].min(dim=0)[0]
            max_val = cano_points[seg_part == child_part_id].max(dim=0)[0]
            print("min_val: {}, max_val: {}".format(min_val, max_val))
            transform = np.eye(4)
            transform[:3, 3] = min_val.cpu().numpy() - EXTEND
            box = o3d.geometry.TriangleMesh.create_box(*(max_val - min_val + (EXTEND * 2)).cpu().numpy())
            box.transform(transform)

            scene = o3d.t.geometry.RaycastingScene()
            scene.add_triangles(o3d.core.Tensor(np.array(box.vertices), dtype=o3d.core.Dtype.Float32),
                                o3d.core.Tensor(np.array(box.triangles), dtype=o3d.core.Dtype.UInt32))

            if all(scene.count_intersections(rays).numpy() == 0):  # no intersection
                if enable_reject:
                    print("Rejecting revolute joint for part {}-{} as axis is not within the object".format(parent_part_id, child_part_id))
                    geo_cost_r = 1e6
                else:
                    print("Revolute joint for part {}-{}'s axis is not within the object".format(parent_part_id, child_part_id))
