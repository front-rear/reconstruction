...
def segment_mesh(dataset_path: str, show_difference: bool = False, show_final_pcd: bool = False) -> None:
    ...
    # Tag parts for SAM3D mesh
    last_mesh = load_mesh(os.path.join(SAM3D_path, f"state_{state_nums[-1]}", "mesh.ply"),
                          os.path.join(oops_path, f"state_{state_nums[-1]}_pose_adjusted.npz"))
    last_SAM3D_labels = np.load(os.path.join(SAM3D_path, f"state_{state_nums[-1]}", "labels.npy"))
    sample_points, face_idx = trimesh.sample.sample_surface_even(last_mesh, 10000)
    sample_labels = last_SAM3D_labels[face_idx]
    map_mesh_label_to_part_label = []
    for i in range(0, np.max(last_SAM3D_labels) + 1):
        mesh_pcd = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(sample_points[sample_labels == i]))
        for part_id in range(len(state_nums)):
            part_pcd = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(final_points[part_labels == part_id]))
            if np.mean(mesh_pcd.compute_point_cloud_distance(part_pcd)) < constants.MESH_CASTING_THRESHOLD:
                print(f"Print mesh label {i} with part {part_id}")
                map_mesh_label_to_part_label.append(part_id)
                last_mesh.visual.face_colors[last_SAM3D_labels == i] = (np.array(list(constants.COLOR_MAP[part_id]) + [1]) * 255).astype(np.uint8)
                break
    map_mesh_label_to_part_label = np.array(map_mesh_label_to_part_label)
    ...
    # Extract each part of SAM3D mesh as collision mesh
    for part_id in range(len(state_nums)):
        part_mesh = last_mesh.submesh(np.where(np.in1d(last_SAM3D_labels, np.where(map_mesh_label_to_part_label == part_id)[0])))[0]
    ...
    # Extract each part of texture mesh
    for part_id in range(len(state_nums)):
...
def run_REART(dataset_path: str, farthest_point_sampling: bool = True) -> None:
    ...
    pc = np.stack(pcds + [pcds[-1] for _ in range(constants.REART_POINTCLOUD_COUNT - len(pcds))], axis=0)
    ...
    np.savez(constants.REART_DATA_PATH, segm=np.tile(labels, (4, 1)), pc=pc, trans=raw_data["trans"])
    ...
    os.system(f"python run_sapien.py --sapien_idx=212 --save_root=exp --n_iter=2000 --cano_idx={cano_idx} --use_flow_loss --use_nproc --use_assign_loss")

    # Run REART Pass 2
    os.chdir(constants.REART_PATH)
    os.system(f"python run_sapien.py --sapien_idx=212 --save_root=exp --n_iter=200 --cano_idx={cano_idx} " +
              "--model=kinematic --use_flow_loss --use_nproc --use_assign_loss --assign_iter=0 --assign_gap=1 --snapshot_gap=10 --base_result_path=exp/sapien_212/result.pkl")
